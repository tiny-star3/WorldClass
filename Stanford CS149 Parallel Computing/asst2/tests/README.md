...
This test allocates two buffers of size 2^15 elements each, and copies elements from one buffer to another in alternating order(交替顺序). Each task will increment values in the input buffer before copying the data to the output buffer. There are 64 tasks and 400 bulk task launches.
...
This test computes a Fibonacci number using a very compute-intensive recursive(递归) method. The computation is specified with 30 bulk task launches, each with 256 tasks.
