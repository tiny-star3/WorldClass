...
shadePixel(int circleIndex, float2 pixelCenter, float4 p, float4* imagePtr) {
...
CudaRenderer::render() {
    ...
    kernelRenderPixels<<<gridDim, blockDim, blockDim.x*blockDim.y*(sizeof(float3)+sizeof(float)+sizeof(int)+4*sizeof(uint))+sizeof(int)>>>();
    cudaDeviceSynchronize();
}
